...
//        get { return this[x * Height + y]; }
//        set { this[x * Height + y] = value;}
    }
    /*
    /// <summary>
    /// 1d indexer to this bitmap
    /// </summary>
    /// <param name="i">index of the bit</param>
    /// <returns>if that bit is true of false</returns>
    private bool this[int i]
    {
        get {
            return ((bitmap[i / 4] & 1 << i % 4) == 1 << i % 4);
        }
        set {
            bitmap[i / 4] = (byte)((value) ?
                bitmap[i / 4] |  (1 << i % 4) :
                bitmap[i / 4] & ~(1 << i % 4));
        }
    }*/
